...
class TextUtil:
    ...
    def randomCharacter(self, binary):
        low = 0 if binary else self.lowText
...
class FileManager:
    ...
    def compareFiles(self, sourcePath, responsePath):
        ...
        while match and not done:
            sc = sfile.read(1)
            rc = rfile.read(1)
            if sc == '' and rc == '':
                # Came to end of both files
                done = True
            elif sc == '':
                match = False
                reason = "Source file %s is shorter (%d bytes) than response file %s" % (sourcePath, byteCount, responsePath)
            elif rc == '':
                match = False
                reason = "Response file %s is shorter (%d bytes) than source file %s" % (responsePath, byteCount, sourcePath)
            elif sc != rc:
                match = False
                sstring = self.textHandler.showCharacter(sc, binary)
                ostring = self.textHandler.showCharacter(rc, binary)
                reason = "Mismatch between source file %s and response file %s at position %d: %s != %s" % (sourcePath, responsePath, byteCount+1, sstring, ostring)
            else:
                # Characters match.  Keep going
                byteCount += 1
